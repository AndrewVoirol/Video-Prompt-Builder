...
export function PresetSidebar({
  ...
  const presetsByCategory = filteredPresets.reduce(
    (acc, preset) => {
  ...
  // Favorite presets
  const favoritePresets = ALL_PRESETS.filter((preset) =>
  ...
  const copyPreset = useCallback(
    (preset: VideoPreset, event: React.MouseEvent) => {
      event.stopPropagation();
      ...
      {Object.entries(presetsByCategory).map(([category, presets], index) => {
        ...
        return (
          <React.Fragment key={category}>
            {(index > 0 || favoritePresets.length > 0) && (
              <SidebarSeparator className="my-2" />
            )}
            <SidebarGroup>
              <SidebarGroupLabel
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {preset.tags.slice(0, 3).map((tag) => (
                        <Badge
                          key={tag}
                          variant="outline"
                          className="text-xs h-4 px-1 py-0 text-sidebar-foreground/60 border-sidebar-border/50"
                        >
                          {tag}
                        </Badge>
                      ))}
                      {preset.tags.length > 3 && (
                        <Badge
                          variant="outline"
                          className="text-xs h-4 px-1 py-0 text-sidebar-foreground/40 border-sidebar-border/30"
                        >
                          +{preset.tags.length - 3}
                        </Badge>
                      )}
