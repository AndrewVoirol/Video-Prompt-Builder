...
export function toMarkdown(state: BuilderState): string {
  ...
  if (Object.keys(state.parameters).length > 0) {
    ...
    Object.entries(state.parameters).forEach(([key, value]) => {
      if (value !== undefined && value !== null) {
        ...
        const sourceInfo = provenance ? ` *(${provenance.source}${provenance.origin ? `: ${provenance.origin}` : ''}${provenance.modified ? ', modified' : ''})*` : '';
  ...
  if (provenanceEntries.length > 0) {
    ...
    provenanceEntries.forEach(([field, prov]) => {
      lines.push(`- **${field}:** ${prov.source}${prov.origin ? ` (${prov.origin})` : ''}${prov.modified ? ' - modified by user' : ''}`);
    });
    lines.push('');
  }
...
export function toNatural(state: BuilderState): string {
  ...
  if (params.aspectRatio) naturalParams.push(`${params.aspectRatio} aspect ratio`);
  if (params.cameraMovement) naturalParams.push(`${params.cameraMovement} camera movement`);
  if (params.lighting) naturalParams.push(`${params.lighting} lighting`);
  if (params.motionIntensity) naturalParams.push(`${params.motionIntensity} motion intensity`);
  if (params.colorGrading) naturalParams.push(`${params.colorGrading} color grading`);
