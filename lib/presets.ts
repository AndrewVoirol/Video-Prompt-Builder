...
export const CINEMATIC_PRESETS: VideoPreset[] = [
  {
    ...
    description:
      "Epic establishing shot with dramatic camera movement and lighting",
    category: "cinematic",
    model: "sora",
    parameters: {
      style: "cinematic",
    ...
    promptTemplate:
      "Cinematic {subject} shot, {environment}, dramatic lighting, {camera_movement} camera movement, golden hour, film grain, shallow depth of field, {mood} atmosphere",
  ...
  },
  {
    ...
    promptTemplate:
      "Close-up of {character}, {emotion} expression, {lighting} lighting, subtle {micro_expression}, {background} background, intimate {mood}",
...
export const DOCUMENTARY_PRESETS: VideoPreset[] = [
  {
    ...
    promptTemplate:
      "Documentary footage of {subject}, {activity}, natural lighting, handheld camera, authentic {environment}, candid {behavior}",
...
export const COMMERCIAL_PRESETS: VideoPreset[] = [
  {
    ...
    promptTemplate:
      "Professional {product} showcase, {background} background, soft studio lighting, {camera_movement}, clean composition, {brand_style} aesthetic",
...
export const ARTISTIC_PRESETS: VideoPreset[] = [
  {
    ...
    promptTemplate:
      "Abstract {pattern} motion, {colors} color palette, {movement_type} movement, {texture} textures, {rhythm} rhythm, experimental visual art",
...
export const getPresetsByCategory = (
  category: VideoPreset["category"],
): VideoPreset[] => {
  return ALL_PRESETS.filter((preset) => preset.category === category);
};

export const getPresetsByModel = (
